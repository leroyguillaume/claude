...
tagged operations, the OpenAPI document at `/openapi.json` rendered by Scalar at
`/docs`). This skill is the Rust-specific stack and its wiring.

**Always:**

- **Every HTTP server / API uses the same stack, always: `axum` + `aide` — no
  exceptions.** Never expose a bare `axum::Router` for an API: build the router
  with `aide`'s `ApiRouter` (drop-in for `axum`), and document every endpoint.
- Generate the OpenAPI document with **`aide`** (its `ApiRouter` drop-in for
  ...
  DTO). `api-conventions` says where it is served and what renders it.
...
- Enable the `aide` features `axum, axum-json, macros` — the base `axum`
  feature does **not** make `axum::Json` an `OperationInput`/`Output`, so
  without `axum-json` no JSON handler satisfies `OperationHandler`.
...
- `OperationInput` for `Query<T>` is behind `aide`'s **`axum-query`** feature.
  ...
  `axum, axum-json, axum-query, macros`. A custom extractor likewise needs a
  (no-op) `impl OperationInput`.
